...
/**
 * POST /api/analyze
 * Body: { observation: string, autoDispatch?: boolean, engine?: "modal" | "ollama" | "auto" | "fallback", reporter?: any }
 */
export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { observation, autoDispatch = true, engine = "auto", reporter } = body;
