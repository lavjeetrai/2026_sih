...
import type { CardData, ColumnData, Priority, Tag } from "@/types";
import { CardDetailModal } from "@/components/hse/card-detail-modal";
import { BulkCsvUploadModal } from "./bulk-csv-upload-modal";
    ...
    <div className="w-full h-full bg-neutral-50 p-6 md:p-8 transition-colors flex flex-col font-sans overflow-hidden">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex items-center gap-1.5 bg-neutral-200/65 p-1 rounded-xl border border-neutral-300/80 self-start sm:self-auto shadow-2xs">
          ...
          <div className="h-4 w-[1px] bg-neutral-300 mx-0.5" />
          <button
            type="button"
            onClick={() => setIsUploadModalOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-neutral-900 text-white hover:bg-neutral-800 shadow-xs cursor-pointer transition-colors"
            title="Import safety observations in bulk from CSV"
          >
            <UploadCloud size={13} className="text-neutral-300" />
            Import CSV
          </button>
        </div>
      </div>
