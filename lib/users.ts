...
// Default initial profiles ensuring the system is immediately usable for demo testing
...
/**
 * Sync local JSON files for offline resilience
 */
async function syncLocalUsers(users: UserDocument[]): Promise<void> {
  try {
    ...
    // Also extract and write approvals registry
...
/**
 * Load users from file if in memory is empty or initialize
 */
...
/**
 * Get all users from MongoDB Atlas, falling back to local storage
 */
export async function getAllUsers(): Promise<UserDocument[]> {
  try {
    ...
    // Seed defaults into Atlas if empty
...
/**
 * Submit an account creation verification request (status: "pending")
 */
export async function createUserRequest(
  payload: Omit<UserDocument, "_id" | "status" | "createdAt" | "updatedAt" | "approval" | "role"> & {
    requestedRole: "worker" | "manager";
  }
...
/**
 * Manager approves user request and assigns role ("worker" | "manager")
 * Maintains the permanent common record of which manager approved whom.
 */
export async function approveUserRequest(params: {
  ...
  try {
    ...
    // Also push to permanent approvals_audit collection
...
/**
 * Manager rejects applicant request
 */
...
/**
 * Returns the permanent common record of which manager approved whom
 */
